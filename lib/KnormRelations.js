...
class KnormRelations {
  ...
  updateQuery(knorm) {
    ...
    class RelationsQuery extends Query {
      ...
      asJoinQuery(asJoinQuery = true) {
        return this.setOption('asJoinQuery', !!asJoinQuery);
      }
      ...
      async prepareJoin(sql, options) {
        ...
        const asJoinQuery = this.getOption('asJoinQuery');

        if (asJoinQuery) {
          this.setOption('ensureUniqueField', false);
        } else {
